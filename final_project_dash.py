...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='Launch Site', 
        title='Total Success Launchs for All Sites')
        return fig
    else:
        filtered_df = spacex_df[spacex_df['Launch Site']==entered_site].groupby('class').count()
        filtered_df['class']=filtered_df.index
        fig = px.pie(filtered_df, values='Launch Site', 
        names='class', 
        title='Total Success Launchs for site {}'.format(str(entered_site)))
        return fig
...
def get_scatter_chart(entered_site,payload_values):
    ...
    if entered_site == 'ALL':
        ...
        filtered_df['Filtered Version']=filtered_df['Booster Version'].apply(lambda x: x.split()[1])
        fig = px.scatter(filtered_df, x='Payload Mass (kg)',y='class',color='Filtered Version')
        return fig
    else:
